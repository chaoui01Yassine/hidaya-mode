...
class ApiException extends \Exception
{
     ...
     * @throws \Mollie\Api\Exceptions\ApiException
     */
    public static function createFromRequestException($requestException, Throwable $previous = null)
    {
        // Not all Guzzle Exceptions implement hasResponse() / getResponse()
        if(method_exists($requestException, 'hasResponse') && method_exists($requestException, 'getResponse')) {
            if ($requestException->hasResponse()) {
                return static::createFromResponse($requestException->getResponse());
            }
        }

        return new static($requestException->getMessage(), $requestException->getCode(), null, $previous);
    }

    /**
     * @param \Hough\Psr7\Response $response
     * @param \Throwable|null $previous
     *
     * @return ApiException
     *
     ...
     * @throws \Mollie\Api\Exceptions\ApiException
     */
    protected static function parseResponseBody(Response $response)
    {
