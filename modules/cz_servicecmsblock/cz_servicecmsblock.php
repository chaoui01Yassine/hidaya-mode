...
class Cz_Servicecmsblock extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('CZ - Service CMS Block', array(), 'Modules.ServiceCmsBlock');
        $this->description = $this->trans('Adds custom information block in your store.', array(), 'Modules.ServiceCmsBlock');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                ...
                `text` text NOT NULL,
                PRIMARY KEY (`id_czservicecmsblockinfo`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;'
        );
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czservicecmsblockinfo`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czservicecmsblockinfo_lang`');
        }
   ...
   public function processSaveServiceCmsBlock()
    {
		...
		if (Shop::isFeatureActive() && !$czservicecmsblockinfo->id_shop) {
            $saved = true;
            $shop_ids = Shop::getShops();
            foreach ($shop_ids as $id_shop) {
                $czservicecmsblockinfo->id_shop = $id_shop;
                $saved &= $czservicecmsblockinfo->add();
            }
        } else {
			$czservicecmsblockinfo->id_shop = Shop::getContextShopID();
    ...
    protected function renderForm()
    {
        ...
        $fields_form = array(
            ...
            'input' => array(
                'id_czservicecmsblockinfo' => array(
                    'type' => 'hidden',
    ...
    {
        $fields_value = array();
        $id_czservicecmsblockinfo = 1;

        foreach (Language::getLanguages(false) as $lang) {
            $czservicecmsblockinfo = new ServiceCmsBlock((int)$id_czservicecmsblockinfo);
            $fields_value['text'][(int)$lang['id_lang']] = $czservicecmsblockinfo->text[(int)$lang['id_lang']];
        }

        $fields_value['id_czservicecmsblockinfo'] = $id_czservicecmsblockinfo;
    ...
    {
        $sql = 'SELECT r.`id_czservicecmsblockinfo`, r.`id_shop`, rl.`text`
            FROM `'._DB_PREFIX_.'czservicecmsblockinfo` r
            LEFT JOIN `'._DB_PREFIX_.'czservicecmsblockinfo_lang` rl ON (r.`id_czservicecmsblockinfo` = rl.`id_czservicecmsblockinfo`)
